...
fetch-data:
	...
	@echo git clone https://github.com/EduardRosert/docker-skinnywms.git /app/docker-skinnywms/
	@echo "from skinnywms.wmssvr import application">/app/docker-skinnywms/demo.py
	@echo "application.run(debug=True, threaded=False, host='0.0.0.0',port=443)">>/app/docker-skinnywms/demo.py
	@mkdir -p /app/docker-dwd-open-data-downloader
        @echo git clone https://github.com/EduardRosert/docker-dwd-open-data-downloader /app/docker-dwd-open-data-downloader
	@mkdir -p /app/data
...
run-application:
	...
	@sh -c "$(CONDA) init bash; export MAGPLUS_HOME=/opt/miniconda3; $(PYTHON) /app/docker-skinnywms/demo.py --host='0.0.0.0' --port=443 --path /app/data >/dev/null 2>&1 &"
	@echo ""
